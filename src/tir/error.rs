...
impl<'base> TirError<'base> {
    pub fn already_defined(position: Range<usize>, source: Rc<SourceFile<'base>>) -> Self {
        TirError::AlreadyDefined { position, source }
    }

    pub fn get_error(&self) -> (Range<usize>, String, Rc<SourceFile<'_>>) {
        match self {
            TirError::ModuleNotFound { module: _, position, source } => (position.clone(), format!("{}", self), source.clone()),
            TirError::ModuleAlreadyDefined { source } => (0..0, format!("{}", self), source.clone()),
            TirError::AstModuleAlreadyDefined { position, source } => (position.clone(), format!("{}", self), source.clone()),
            TirError::AlreadyDefined { position, source } => (position.clone(), format!("{}", self), source.clone()),
            TirError::TypeNotFound { source, position } => (position.clone(), format!("{}", self), source.clone()),
        }
    }
}