...
impl<'a> ElfSectionHeader<'a> {
    pub fn parse(
        ...
        self.sh_type =
            unsafe { core::mem::transmute::<u32, SectionHeaderType>(reader.fetch_u32()?) };
        self.sh_flags = unsafe {
            core::mem::transmute::<u64, SectionHeaderFlag>(match bit_mode {
...
impl ElfHeader {
    ...
    pub fn parse(&mut self, reader: &mut BufferReader) -> Result<(), BinaryFormatError> {
        if reader.fetch_u8()? != 0x7f
            || reader.fetch_u8()? != 0x45
            || reader.fetch_u8()? != 0x4c
            || reader.fetch_u8()? != 0x46
        {
...
impl<'a> BinaryFormat<'a> for ElfFormat<'a> {
    fn parse(reader: &'a mut BufferReader) -> Result<Self, BinaryFormatError> {
        ...
        reader.set_index(
            usize::from(elf_header.e_shoff)
                + (elf_header.e_shentsize * elf_header.e_shstrndx) as usize
                + match elf_header.bit_mode {
                    BitMode::_32 => 0x10,
                    BitMode::_64 => 0x18,
                },
        )?;
        ...
        for _ in 0..elf_header.e_shnum {
            ...
            section_header.name = unsafe {
                str_from_null_terminated_utf8(&string_data[(section_header.sh_name as usize)..])?
            };
        ...
        let text_section = section_headers
            .iter()
        ...
        let codes = match text_section {
            Some(section) => {
                ...
                let text_relas =
                    Self::find_rela(&section_headers, elf_header.bit_mode, ".rela.text", reader)?;
...
impl<'a> ElfFormat<'_> {
    fn find_rela(
        ...
        name: &str,
        reader: &mut BufferReader,
    ) -> Result<Option<Vec<ElfRela>>, BinaryFormatError> {
